...
export default function QueryEnginePage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-7rem)] gap-8">
      ...
      {/* Dynamic Grid Layout */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 flex-1 h-full min-h-0 overflow-hidden">
        {/* Main Interaction - Chat Area */}
        <div className="lg:col-span-3 flex flex-col h-full min-h-0">
          <Card className="flex-1 flex flex-col overflow-hidden bg-card/50 backdrop-blur-sm border-border/40 shadow-xl overflow-hidden">
            <CardContent className="flex flex-col h-full p-0 overflow-hidden">
                                          ...
                                          <div className="rounded-2xl border border-border/50 bg-background/50 p-4 transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:-translate-y-0.5 cursor-pointer">
                                            <div className="flex items-center justify-between mb-2">
                                              <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-sm font-bold text-foreground/80 group-hover:text-primary transition-colors">{src.speaker || "Unknown"}</span>
                                              </div>
                                              {src.emotion && (
                                            ...
                                            <p className="text-sm text-muted-foreground/80 line-clamp-2 leading-relaxed italic group-hover:text-foreground/90 transition-colors">
                                              "{src.text}"
                                            </p>
                                          </div>
                                        </Link>
              ...
              {/* Input Area */}
              <div className="border-t border-border p-4 flex-shrink-0 bg-background/50 backdrop-blur-md">
                <div className="flex items-center gap-3 rounded-xl border border-border bg-muted/30 px-4 py-3 transition-all focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20">
                  ...
                  />
                  <Button size="icon" disabled={isThinking || chatMutation.isPending || !query.trim()} className="h-8 w-8 rounded-full bg-primary flex-shrink-0 shadow-lg shadow-primary/20" onClick={() => handleSubmit()}>
        ...
        {/* Sidebar - Context & Suggestions aligned top-row */}
        <div className="lg:col-span-1 flex flex-col gap-6 overflow-y-auto pr-1 scrollbar-hide">
          {/* How it Works Card */}
          <Card className="border-primary/5 bg-primary/5 shadow-none pb-2 flex-shrink-0">
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center gap-2 pb-1 border-b border-primary/10">
                <Sparkles className="h-3.5 w-3.5 text-primary" />
                <p className="text-sm font-bold uppercase tracking-wider text-foreground">How it works</p>
              </div>
              <div className="space-y-4 text-[13px] leading-relaxed">
                <div className="flex gap-3.5 group/step">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-background text-[11px] font-bold text-primary shadow-sm border border-primary/20 group-hover/step:bg-primary group-hover/step:text-primary-foreground transition-all duration-300">1</div>
                  <p className="text-muted-foreground/90 font-medium">LLM classifies query intent into Focused vs. Global modes.</p>
                </div>
                <div className="flex gap-3.5 group/step">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-background text-[11px] font-bold text-primary shadow-sm border border-primary/20 group-hover/step:bg-primary group-hover/step:text-primary-foreground transition-all duration-300">2</div>
                  <p className="text-muted-foreground/90 font-medium">Two-stage retrieval: Pinecone for recall, BGE-Reranker for precision.</p>
                </div>
                <div className="flex gap-3.5 group/step">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-background text-[11px] font-bold text-primary shadow-sm border border-primary/20 group-hover/step:bg-primary group-hover/step:text-primary-foreground transition-all duration-300">3</div>
                  <p className="text-muted-foreground/90 font-medium">Diversity sampling ensures balanced multi-meeting representation.</p>
                </div>
                <div className="flex gap-3.5 group/step">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-background text-[11px] font-bold text-primary shadow-sm border border-primary/20 group-hover/step:bg-primary group-hover/step:text-primary-foreground transition-all duration-300">4</div>
                  <p className="text-muted-foreground/90 font-medium">Sigmoid normalization provides probabilistic confidence scoring.</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-3 flex-shrink-0">
            <h3 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/60 px-1">Suggested Queries</h3>
            <div className="space-y-2">
              {suggestedQueries.map(s => (
                <button 
                  key={s.title} 
                  className="w-full text-left p-3 rounded-xl border border-border/40 bg-card/30 hover:bg-primary/5 hover:border-primary/30 transition-all group shadow-sm"
                  onClick={() => {
          ...
          <div className="pt-2">
            {messages.length > 0 && (
              <Button variant="ghost" size="sm" className="w-full text-muted-foreground hover:bg-red-500/5 hover:text-red-500 rounded-full text-xs font-bold uppercase tracking-widest" onClick={() => setMessages([])}>Clear conversation</Button>
            )}
          </div>
        </div>
      </div>
